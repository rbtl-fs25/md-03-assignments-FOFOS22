...
sanitation_nameofmychoice <- sanitation |> 
  filter(iso3 == "AND") |> 
  filter(year %in% c(2000,2020) ) |> 
  filter(varname_short == "san_sm")
```
...
```{r}
ggplot(data = sanitation_nameofmychoice, 
       aes( x = residence,
            y = percent,
            fill = varname_long)) +
  geom_col() +
...
2.  What do you observe when you look at the code and plot? -\> no y-axis is applied
